...
import '../style/HeaderPaginaAdmin.css'
...
const HeaderPaginaAdmin = ({ onBuscar }: { onBuscar?: (termino: string) => void }) => {
    const { user, logout } = useContext(AuthContext)
    const { pedidosSinRevisar } = usePedidos()
            ...
            <header className="main-header">
                <div className="header-top">
                    ...
                    <div className="user-actions">
                        <div
                            className={`user-name ${isPanelOpen ? 'open' : ''}`}
                        ...
                        <div className={`user-panel ${isPanelOpen ? 'open' : ''}`}>
                            <button
                                ...
                                onClick={() => { navigate('/InformacionAdmin'); setIsPanelOpen(false) }}
                            >
                                Información del Administrador
                            </button>
                            <button className="user-panel-btn logout" onClick={logout}>Cerrar Sesión</button>
                        </div>
                    </div>
                </div>
                ...
                <nav className="nav-menu">
                    ...
                    {/* badge de pedidos sin revisar */}
                    <Link to="/UltimosPedidos" className="nav-item nav-item--con-badge">
                        <span className="nav-icon">📝</span>
                        Últimos Pedidos
                        {pedidosSinRevisar > 0 && (
                            <span className="nav-badge">
                                {pedidosSinRevisar > 99 ? '99+' : pedidosSinRevisar}
                            </span>
                        )}
                    </Link>
                </nav>
...
export default HeaderPaginaAdmin